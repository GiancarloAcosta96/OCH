...
$('#tablaDatatable').load('../capa_presentacion/tabla_nc_fija_back_global.php?ano='+ano+'&periodo='+periodo+'&ncestadof='+ncestadof+'&tienda='+tienda);
          ...
          if (datos.length > 0) {
            $("#nczonaltdp").append("<option value = '0'> --Elegir-- </option>");
                ...
                for (i = 0; i < datos.length; i++) {
                    ...
                    $("#nczonaltdp").append("<option value = '" + datos[i][0] + "' selected>" + datos[i][1] + "</option>");
                    }else{
                        $("#nczonaltdp").append("<option value = '" + datos[i][0] + "'>" + datos[i][1] + "</option>");
                    }
    ...
    $.ajax({
        ...
        url: "../procesos/obtenDatosEjecutivoCartera.php",
         success: function (r) {
            datos = jQuery.parseJSON(r);
          if (datos.length > 0) {
            $("#ncejecutivotdp").append("<option value = '0'> --Elegir-- </option>");
                var i;
                for (i = 0; i < datos.length; i++) {

                    if (datos[i][0] == idcartera) {
                    $("#ncejecutivotdp").append("<option value = '" + datos[i][0] + "' selected>" + datos[i][2] + "</option>");
                    }
        ...
        url: "../procesos/obtenDatosEjecutivo.php",
         success: function (data) {
          ...
          if (datos.length > 0) {
                ...
                var i;
                for (i = 0; i < datos.length; i++) {
                    $("#ncejecutivotdp").append("<option value = '" + datos[i][0] + "'>" + datos[i][2] + "</option>");
                }
            }
        }
    });
